...
import com.aircompany.reviewapi.repository.FlightRepository;
...
class FlightServiceTest {
    ...
    @Test
    void getFlightByFlightNumber_shouldHandleNullInput() {
        // Given
        when(flightRepository.findByFlightNumber(null)).thenReturn(null);

        // When
        Flight result = flightService.getFlightByFlightNumber(null);

        // Then
        assertNull(result);
        verify(flightRepository, times(1)).findByFlightNumber(null);
    }

    @Test
    void getFlightsByCompanyName_shouldHandleNullInput() {
        // Given
        when(flightRepository.findByCompanyName(null)).thenReturn(Arrays.asList());

        // When
        List<Flight> result = flightService.getFlightsByCompanyName(null);

        // Then
        assertNotNull(result);
        assertTrue(result.isEmpty());
        verify(flightRepository, times(1)).findByCompanyName(null);
    }
}
